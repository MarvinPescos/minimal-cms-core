...
@pytest.fixture
async def session(db_session_factory) -> AsyncGenerator[AsyncSession, None]:
    """Get Fresh DB session for each test"""
    async with db_session_factory() as session:
        yield session 

@pytest.fixture
async def client(session) -> AsyncGenerator[AsyncClient, None]:
    """Fake API client that overides DB dependency"""
...
@pytest.fixture
def mock_user_token(client, test_user):
